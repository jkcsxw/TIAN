...
" "$CATALOG" 2>/dev/null || true)
    ...
    # bash 3.2 (macOS) chokes on ';;' tokens inside a heredoc-within-$(...), so we
    # use a helper function whose top-level heredoc avoids that parse error entirely.
    # Dynamic IDs are injected via placeholder substitution afterward.
    _tian_bash_completion_template() {
cat <<'ENDOFSCRIPT'
...
_tian_complete() {
    ...
    case "$cmd" in
        run)
            case "$prev" in
                --backend) COMPREPLY=( $(compgen -W "__BACKEND_IDS__" -- "$cur") ) ;;
        ...
        install)
            case "$prev" in
                --backend) COMPREPLY=( $(compgen -W "__BACKEND_IDS__" -- "$cur") ) ;;
                --mcp)     COMPREPLY=( $(compgen -W "__MCP_IDS__ default" -- "$cur") ) ;;
                --skills)  COMPREPLY=( $(compgen -W "__SKILL_IDS__" -- "$cur") ) ;;
        ...
        add)
            if [[ $cword -eq 2 ]]; then
                ...
                case "${COMP_WORDS[2]}" in
                    mcp)   COMPREPLY=( $(compgen -W "__MCP_IDS__" -- "$cur") ) ;;
                    skill) COMPREPLY=( $(compgen -W "__SKILL_IDS__" -- "$cur") ) ;;
                esac
            fi ;;
        remove)
            if [[ $cword -eq 2 ]]; then
                ...
                COMPREPLY=( $(compgen -W "__MCP_IDS__" -- "$cur") )
            fi ;;
        list)
        ...
        skill)
            if [[ $cword -eq 2 ]]; then
                ...
                case "${COMP_WORDS[2]}" in
                    run|info) COMPREPLY=( $(compgen -W "__SKILL_IDS__" -- "$cur") ) ;;
                esac
            fi ;;
        key)
            if [[ $cword -eq 2 ]]; then
                ...
                COMPREPLY=( $(compgen -W "__BACKEND_IDS__" -- "$cur") )
            fi ;;
        config)
            if [[ $cword -eq 2 ]]; then
...
ENDOFSCRIPT
    ...
    local bash_script
    bash_script=$(_tian_bash_completion_template)
    bash_script="${bash_script//__BACKEND_IDS__/${_backend_ids}}"
    bash_script="${bash_script//__MCP_IDS__/${_mcp_ids}}"
    bash_script="${bash_script//__SKILL_IDS__/${_skill_ids}}"
