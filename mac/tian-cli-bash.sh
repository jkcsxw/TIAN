...
" "$CATALOG" 2>/dev/null || true)
    ...
    # Use a quoted heredoc (<<'ENDOFSCRIPT') so bash 3.2 (macOS) doesn't choke on \$( inside
    # a heredoc-within-$(...). Dynamic IDs are injected via placeholder substitution afterward.
    local bash_script
