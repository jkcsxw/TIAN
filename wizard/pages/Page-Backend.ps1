function Show-Page-Backend {
    ...
    $title = New-Label -Text "Choose Your AI Assistant" -X 50 -Y 30 -Width 460 -Height 38 -Font $UI_FONT_TITLE -ForeColor $UI_COLOR_ACCENT
    $Panel.Controls.Add($title)

    $sub = New-Label -Text "Select the AI you want to use. You can change this later by re-running setup." -X 50 -Y 70 -Width 460 -Height 22 -Font $UI_FONT_BODY -ForeColor $UI_COLOR_MUTED
    ...
    foreach ($backend in $Catalog.backends) {
        ...
        $rb = New-RadioButton -Text $backend.displayName -X 50 -Y $y -Width 300 -Height 22 -Checked $isSelected
        $rb.Font = New-Object System.Drawing.Font("Segoe UI", 10, [System.Drawing.FontStyle]::Bold)
        $rb.Tag = $backend.id
        $Panel.Controls.Add($rb)
        $radioButtons += $rb

        $descLbl = New-Label -Text $backend.description -X 72 -Y ($y + 22) -Width 420 -Height 36 -Font $UI_FONT_SMALL -ForeColor $UI_COLOR_MUTED -Wrap $true
        $Panel.Controls.Add($descLbl)

        if ($backend.apiKeyUrl) {
            $link = New-LinkLabel -Text "Get API key" -X 72 -Y ($y + 58) -Width 120 -Height 18 -Url $backend.apiKeyUrl
