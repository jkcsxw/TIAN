function Show-Page-McpServers {
    ...
    $title = New-Label -Text (T "mcp.title") -X 50 -Y 20 -Width 460 -Height 38 -Font $UI_FONT_TITLE -ForeColor $UI_COLOR_ACCENT
    $Panel.Controls.Add($title)

    $sub = New-Label -Text (T "mcp.subtitle") -X 50 -Y 58 -Width 460 -Height 22 -Font $UI_FONT_BODY -ForeColor $UI_COLOR_MUTED
    ...
    foreach ($group in $groups) {
        ...
        foreach ($server in $group.Group) {
            ...
            $cb = New-CheckBox -Text (Get-DisplayName $server) -X 10 -Y $y -Width 200 -Height 22 -Checked $isChecked
            $cb.Tag = $server.id
            $scrollPanel.Controls.Add($cb)
            $checkBoxes[$server.id] = $cb

            $descLbl = New-Label -Text (Get-Description $server) -X 220 -Y $y -Width 220 -Height 36 -Font $UI_FONT_SMALL -ForeColor $UI_COLOR_MUTED -Wrap $true
